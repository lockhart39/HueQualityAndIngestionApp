...
.PHONY: install
install: virtual-env install-check install-core-structure install-desktop install-apps install-env install-dataIngest

.PHONY: install-dataIngest
install-dataIngest:
	@echo '--- Installing dataIngest Application...'
	cp -R $(ROOT)/dataIngest $(INSTALL_DIR)/dataIngest
	$(INSTALL_DIR)/build/env/bin/python $(INSTALL_DIR)/tools/app_reg/app_reg.py --install $(INSTALL_DIR)/dataIngest	
